...
//	private boolean saveCheckpoints = true;
	...
	public void init() {
		...
		if(!headless()) {
			...
			dialog.updateProgressText(TrainingSteps.PREPARATION, "Loading TensorFlow" );
		}
	...
	public void train() {
		...
		if (!headless()) dialog.updateProgressText(TrainingSteps.PREPARATION, "Creating session" );
		try (Graph graph = new Graph();
			...
			if(!headless()) dialog.updateProgressText(TrainingSteps.PREPARATION, "Preparing data for training");
...
//			List<RandomAccessibleInterval<FloatType>> targets = new ArrayList<>();
			...
			if (!headless()) dialog.updateProgressText(TrainingSteps.PREPARATION, "Starting training ...");

			// Create dialog
			if (!headless()) dialog.updateTrainingStatus( TrainingSteps.PREPARATION, TrainingStepStatus.DONE);
			if (!headless()) dialog.updateTrainingStatus( TrainingSteps.TRAINING, TrainingStepStatus.RUNNING);
...
//			sess.runner().feed("save/Const", checkpointPrefix).addTarget("save/control_dependency").run();
			...
			if (!headless()) dialog.updateTrainingStatus( TrainingSteps.TRAINING, TrainingStepStatus.DONE);
...
//		uiService.show("tiles", tilesStack);
	...
	public void addTrainingAndValidationData(RandomAccessibleInterval<FloatType> training, double validationAmount) {
		...
		if (!headless()) dialog.updateProgressText(TrainingSteps.PREPARATION, "Normalize and tile training and validation data.." );
	...
	public void addTrainingData(RandomAccessibleInterval<FloatType> training) {
		...
		if (!headless()) dialog.updateProgressText(TrainingSteps.PREPARATION, "Normalizing and tiling training data" );
	...
	public void addValidationData(RandomAccessibleInterval<FloatType> validation) {
		...
		if (!headless()) dialog.updateProgressText(TrainingSteps.PREPARATION, "Normalizing and tiling validation data" );
