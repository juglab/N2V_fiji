...
//			int stepsPerEpoch = n_train / trainBatchSize;
			...
			N2VDataWrapper< FloatType > validation_data =
					new N2VDataWrapper<>( context, _validationX, ( int ) Math.min( trainBatchSize, _validationX.dimension( 2 ) ), n2v_perc_pix, val_patch_shape, N2VDataWrapper::uniform_withCP );
...
//			List<RandomAccessibleInterval<FloatType>> targets = new ArrayList<>();
			...
			if ( !headless() ) dialog.setCurrentTaskMessage( "Starting training." );

			// Create dialog
			if ( !headless() ) dialog.setTaskDone( 0 );
			if ( !headless() ) dialog.setTaskStart( 1 );
...
//					uiService.show("target", opService.copy().rai(item.getSecond()));
					...
					runner.feed( tensorXOpName, tensorX ).feed( tensorYOpName, tensorY ).feed( learningPhaseOpName, Tensors.create( true ) ).feed( sampleWeightsOpName, tensorWeights ).addTarget(
							trainingTargetOpName );
...
//			if (targets.size() > 0) uiService.show("targets", Views.stack(targets));
	...
	private float validate( Session sess, N2VDataWrapper validationData, Tensor tensorWeights ) {
		...
		runner.feed( tensorXOpName, tensorX ).feed( tensorYOpName, tensorY ).feed( learningPhaseOpName, Tensors.create( false ) ).feed( sampleWeightsOpName, tensorWeights ).addTarget(
				validationTargetOpName );
		runner.fetch( lossOpName );
		runner.fetch( absOpName );
		runner.fetch( mseOpName );
...
//			updateHistoryImage(output);
	...
	private void updateSplitImage( RandomAccessibleInterval< FloatType > in, RandomAccessibleInterval< FloatType > out ) {
		if ( splitImage == null )
			splitImage = opService.copy().rai( out );
		else
			opService.copy().rai( splitImage, out );
		...
		if ( display == null )
			uiService.show( "training preview", splitImage );
		else
			display.update();
	}
	...
	private void updateHistoryImage( RandomAccessibleInterval< FloatType > out ) {
		...
		if ( display == null )
			uiService.show( "training history", stack );
		else {
			display.clear();
...
//		uiService.show("tiles", tilesStack);
	...
	public void addTrainingAndValidationData( RandomAccessibleInterval< FloatType > training, double validationAmount ) {
		...
		if ( !headless() ) dialog.setCurrentTaskMessage( "Normalize and tile training and validation data.." );
	...
	public void addTrainingData( RandomAccessibleInterval< FloatType > training ) {
		...
		if ( !headless() ) dialog.setCurrentTaskMessage( "Normalizing and tiling training data" );
	...
	public void addValidationData( RandomAccessibleInterval< FloatType > validation ) {
		...
		if ( !headless() ) dialog.setCurrentTaskMessage( "Normalizing and tiling validation data" );
